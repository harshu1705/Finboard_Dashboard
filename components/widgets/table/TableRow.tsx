...
export default function TableRowComponent({ row, dynamicFields, onRemoveSymbol }: { row: TableRow, dynamicFields: string[], onRemoveSymbol?: (s: string) => void }) {
  ...
  return (
    <tr className="border-t border-gray-700/20 hover:bg-gray-800/20 transition-colors">
      <td className="px-4 py-3 text-sm font-semibold text-foreground align-middle whitespace-normal break-words">{row.symbol}</td>
      {dynamicFields.map((f) => {
        const val = getNestedValue(row.data, f)
        if (typeof val === 'number' && /price|close|high|low|volume/i.test(f)) {
          return <td key={f} className="px-4 py-3 text-sm text-foreground align-middle whitespace-nowrap">{formatValue(val, getDefaultFormatType('price'))}</td>
        }
        return <td key={f} className="px-4 py-3 text-sm text-foreground align-middle whitespace-normal break-words">{val === undefined || val === null ? '—' : String(val)}</td>
      })}

      <td className="px-4 py-3 text-sm text-muted-foreground align-middle whitespace-normal">
        <div className="flex items-center gap-2">
          {onRemoveSymbol && (
            <button onClick={() => onRemoveSymbol(row.symbol)} onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()} className="rounded-md p-1 hover:bg-red-900/20 hover:text-red-400" title={`Remove ${row.symbol}`}>
      ...
      <td className="px-4 py-3 text-sm text-muted-foreground align-middle whitespace-nowrap">{row.data?.lastUpdated ? new Date(row.data.lastUpdated).toLocaleTimeString() : (row.error ? 'Error' : '—')}</td>
    </tr>
  )
}
